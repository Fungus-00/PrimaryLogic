...
theorem not_not' {p : Prop} : ¬¬p ↔ p := ⟨dne, fun h1 h2 => h2 h1⟩
...
theorem Nat.nonempty_exists_min {s : Set Nat} : s.Nonempty → ∃ n ∈ s, ∀ m < n, m ∉ s := by
  intro ⟨n0, h0⟩
  apply dne; by_contra h'
  simp only [not_exists, not_and, not_forall, not_not'] at h'
  suffices h1 : ∀ n : Nat, ∀ m ∈ s, n < m from (lt_self_iff_false n0).mp <| h1 n0 n0 h0
  intro n m hm
  induction n generalizing m with
  | zero =>
    apply dne; by_contra h1
    rw [Nat.not_lt, Nat.le_zero] at h1
    rw [h1] at hm
    obtain ⟨k, hk, _⟩ := h' 0 hm
    exact Nat.not_lt_zero k hk
  | succ n hn =>
    rcases lem (n + 1 ∈ s) with h1 | h1
    · have ⟨k, hk, h2⟩ := h' (n + 1) h1
      rw [Nat.lt_add_one_iff] at hk
      have h3 := hn k h2
      exfalso
      exact Nat.not_le.mpr h3 hk
    · have h2 := hn m hm
      rw [Nat.lt_iff_add_one_le, Nat.le_iff_lt_or_eq] at h2
      refine Or.resolve_right h2 fun h3 => ?_
      rw [h3] at h1
      exact h1 hm

lemma Nat.eFind {p : Nat → Prop} (H : ∃ n, p n) : ∃ n : Nat, p n ∧ ∀ m < n, ¬p m :=
  have ⟨n, h1, h2⟩ := Nat.nonempty_exists_min H; ⟨n, h1, h2⟩
...
@[reducible]
def Freshable.ofLeftInverse {α β : Type*} {f : α → β} {g : β → α} (v : Function.LeftInverse g f)
...
@[reducible]
def Freshable.ofLeftInverse_Subtype {α : Type*} {f g : α → α}
    (v : Function.LeftInverse g f) [u : Freshable α] : Freshable <| Subtype <| Set.range f :=
  @Freshable.ofLeftInverse α (Subtype (Set.range f))
    (fun x => ⟨f x, x, rfl⟩) (g ∘ Subtype.val) (fun x => Function.comp_apply ▸ v x) _
...
theorem univ (h : Function.Injective f) : PartInj Set.univ f := fun _ _ he => h he
...
theorem ofLeftInverse {f : α → β} {g : β → α} (h : Function.LeftInverse g f) :
    PartInj (Set.range f) g := fun ⟨a, hx⟩ ⟨b, hy⟩ he =>
  hy ▸ hx ▸ (congrArg f <| h b ▸ h a ▸ hy.symm ▸ hx.symm ▸ he)
...
theorem Nat.even_odd_disjoint {m n : Nat} : m * 2 ≠ n * 2 + 1 := by omega
end even_odd
end PrimaryLogic
